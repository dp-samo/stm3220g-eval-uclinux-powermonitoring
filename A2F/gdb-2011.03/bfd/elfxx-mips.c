	...
	*namep = bfd_section_name (input_bfd, sec);
	       ...
	       && h->root.root.u.def.section)
	{
	  if (h->has_plt_entry)
		...
		{
		  isa_bit = 1;
		  is_mips16 = (e_flags & EF_MIPS_ARCH_ASE_MICROMIPS) == 0;
...
_bfd_mips_elf_create_dynamic_sections (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (htab->is_vxworks)
	...
	= ((e_flags & EF_MIPS_ARCH_ASE_MICROMIPS) == 0
  ...
  /* As above, VxWorks requires PLT entries for externally-defined
      ...
      if (!info->shared && !h->def_regular)
	{
      ...
      htab->splt->size = (htab->plt_header_size
			  + htab->plt_mips_offset + htab->plt_comp_offset);
				     ...
				     Elf_Internal_Sym *sym)
{
  ...
  if (h->plt.plist != NULL && hmips->no_fn_stub
	...
	{
	  flagword e_flags = elf_elfheader (output_bfd)->e_flags;
	  bfd_boolean is_mips16 = (e_flags & EF_MIPS_ARCH_ASE_MICROMIPS) == 0;
	  bfd_vma plt_offset;
	  ...
	  if (is_mips16)
	  ...
	  else
	    {
	      ...
	      if (((gotpc_offset & 0x1ffffff) ^ 0x1000000) - 0x1000000
		  != gotpc_offset)
		return FALSE;
				    ...
				    asymbol **ret)
{
  ...
  *ret = NULL;
  ...
  nend = (char *) s + size;
  n = 0;
  for (plt_offset = 4 * ARRAY_SIZE (mips_o32_exec_plt0_entry);
       ...
       plt_offset += entry_size)
    {
      ...
      opcode = bfd_get_16 (abfd, plt_data + plt_offset + 4);
      opcode = (opcode << 16) | bfd_get_16 (abfd, plt_data + plt_offset + 6);
