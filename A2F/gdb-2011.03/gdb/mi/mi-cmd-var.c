	      ...
	      int print_expression)
{
  ...
  if (gdb_type != NULL
      && (TYPE_ANY_FIELD_SENSITIVE (gdb_type)
	  || TYPE_READ_SENSITIVE (gdb_type)))
    ui_out_field_int (uiout, "has-side-effects", 1);
...
create_varobj_in_frame (char *name, char *expression, char *frame)
{
  ...
  if (varobjdebug)
    fprintf_unfiltered (gdb_stdlog,
			...
			name, frame, hex_string (frameaddr), expression);
...
mi_cmd_var_registers (char *command, char **argv, int argc)
{
  ...
  char *frame;
  int non_core = 0;
  VEC (char_p) *register_names = 0;
  enum language prev_lang;
  struct gdbarch *gdbarch;

  if (argc != 1 && argc != 2)
    error (_("mi_cmd_var_registers: Usage: [--non-core] FRAME."));

  if (argc == 2)
    {
      if (strcmp (argv[0], "--non-core") == 0)
	non_core = 1;
      else
  ...
  /* Note that the test for a valid register must include checking the
     register name because gdbarch_num_regs may be allocated for the union of
     the register sets within a family of related processors.  In this
     case, some register names will change depending upon
  ...
  /* Collect the list of core registers, if needed.  */
  if (!non_core)
    {
      for (regnum = 0; regnum < numregs; regnum++)
	{
	  if (gdbarch_register_name (gdbarch, regnum) != NULL
	    ...
	    {
	      char_p name = (char_p) gdbarch_register_name (gdbarch,
							    regnum);
	      VEC_safe_push (char_p, register_names, name);
	    }
	}
    }
  /* Collect the list of memory mapped registers.  */
  {
    const struct target_desc *tdesc = gdbarch_target_desc (gdbarch);

    if (tdesc != NULL)
      tdesc_push_space_names (tdesc, &register_names);
  }

  /* Create variable objects for each register.  */
  {
    int ix;
    char_p name;
    for (ix = 0; VEC_iterate (char_p, register_names, ix, name); ix++)
      {
	char *varobj_name;
	char *expression;
	struct varobj *var;
	...
	varobj_name = varobj_gen_name ();
	make_cleanup (free_current_contents, &varobj_name);
	      
	expression = xstrprintf ("$%s", name);
	make_cleanup (xfree, expression);

	var = create_varobj_in_frame (varobj_name, expression, frame);
	if (var == NULL)
	  error (_("-var-registers: unable to create variable object"));
