		    ...
		    enum language *funlang)
{
  ...
  *funlang = language_unknown;
  ...
  /* If we did not find a function, check in an overlay instead.  This
     is used on MIPS to find functions in KSEG0 / KSEG1 (uncached and
     cached mappings of the same memory).  */
...
/* Read a frame specification in whatever the appropriate format is
   ...
   NULL).  When SELECTED_P is non-NULL set its target to indicate that
   the default selected frame was used.  When CREATED_P is non-NULL set
   its target to indicate whether a new frame has been created.  Make a
   newly-created frame the current one.  */

static struct frame_info *
parse_frame_specification_1 (const char *frame_exp, const char *message,
			     int *selected_frame_p, int *frame_created_p)
{
  int numargs;
  struct value *args[2];
  ...
  /* No new frame by default.  */
  if (frame_created_p != NULL)
    (*frame_created_p) = 0;
  ...
  /* Assume that the single arg[0] is an address, use that to identify
     a frame with a matching ID.  If arg[1] is present, use it as the PC.
     Should this also accept stack/pc/special?  */
  {
    struct frame_id id;
    struct frame_info *fid;

    if (numargs < 2)
      id = frame_id_build_wild (addrs[0]);
    else
      id = frame_id_build (addrs[0], addrs[1]);
  ...
  /* We couldn't identify the frame as an existing frame, but
     perhaps we can create one with the available arguments.  */
  {
    struct frame_info *fid;

    if (numargs < 2)
      fid = create_new_current_frame (addrs[0], 0);
    else
      fid = create_new_current_frame (addrs[0], addrs[1]);
...
{
  return parse_frame_specification_1 (frame_exp, NULL, NULL, NULL);
}

static void
do_restore_frame_chain_cleanup (void *arg)
{
  reinit_frame_cache ();
}
...
frame_info (char *addr_exp, int from_tty)
{
  ...
  fi = parse_frame_specification_1 (addr_exp, "No stack.",
				    &selected_frame_p, &frame_created_p);
  if (frame_created_p)
    make_cleanup (do_restore_frame_chain_cleanup, NULL);
...
{
  select_frame (parse_frame_specification_1 (level_exp, "No stack.",
					     NULL, NULL));
}
...
return_command (char *retval_exp, int from_tty)
{
  ...
  if (retval_exp)
	...
	value_fetch_lazy (return_value);
      ...
      if (thisfun != NULL)
	set_value_address (value, BLOCK_START (SYMBOL_BLOCK_VALUE (thisfun)));
  ...
  if (return_value != NULL)
    {
      ...
      gdb_assert (gdbarch_return_value (gdbarch, func_type, value, NULL,
					NULL, NULL)
		  == RETURN_VALUE_REGISTER_CONVENTION);
      gdbarch_return_value (gdbarch, func_type, value,
