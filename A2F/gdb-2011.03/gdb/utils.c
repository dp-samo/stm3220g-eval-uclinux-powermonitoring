...
/* Compare source code filenames appropriately for the host system.
   We assume that GDB itself is running on the same kind of system the
   compiler is running on.

   On Unix, this is pretty trivial.  On Win32 systems, we fold case,
   and treat slash and backslash as equivalent.  */
int
gdb_filename_cmp (const char *a, const char *b)
{
#ifdef HAVE_DOS_BASED_FILE_SYSTEM
  for (;;)
    {
...
/* Combine an argv vector into a string, in the reverse of buildargv
   from libiberty.  The return value is allocated with xmalloc.  */

char *
make_arg_string (const char **argv)
{
  int i, n, len;
  char *result, *presult;
  ...
  for (i = 0, len = 0; i < n; i++)
      ...
      while ((parg = strpbrk (parg, "\\\"'\t ")))
	{
      ...
      /* We need the length of the argument, any required backslashes,
	 and a trailing space or NUL.  */
      len += strlen (arg) + count + 1;
    }

  result = presult = xmalloc (len);
  for (i = 0; i < n; i++)
    {
      const char *arg, *parg, *prev;

      prev = parg = arg = argv[i];

      while ((parg = strpbrk (parg, "\\\"'\t ")))
	{
	  strncpy (presult, prev, parg - prev);
	  presult += parg - prev;
	  prev = parg++;
	  *presult++ = '\\';
	}
