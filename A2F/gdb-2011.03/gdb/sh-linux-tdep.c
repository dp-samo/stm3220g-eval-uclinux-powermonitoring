...
static void
sh_uclinux_init_abi (struct gdbarch_info info, struct gdbarch *gdbarch)
{
  /* uClinux uses FDPIC, but check just in case.  */
...
_initialize_sh_linux_tdep (void)
{
  ...
  gdbarch_register_osabi (bfd_arch_sh, 0,
			  GDB_OSABI_UCLINUX, sh_uclinux_init_abi);
}
