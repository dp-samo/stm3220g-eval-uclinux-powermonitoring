...
struct block *
dwarf2_finish_block (struct die_info *die, struct dwarf2_cu *cu,
		     struct symbol *symbol, struct pending **listhead,
		     struct pending_block *old_blocks,
		     CORE_ADDR start, CORE_ADDR end,
		     struct objfile *objfile, CORE_ADDR baseaddr)
{
  struct block *block = finish_block (symbol, listhead, old_blocks, start,
				      end, objfile);

  /* Note that recording ranges after traversing children, as we do
     here, means that recording a parent's ranges entails walking
     across all its children's ranges as they appear in the address
     map, which is quadratic behavior.

     It would be nicer to record the parent's ranges before traversing
     its children, simply overriding whatever you find there.  But
     since we don't even decide whether to create a block until after
     we've traversed its children, that's hard to do.  */
  dwarf2_record_block_ranges (die, block, baseaddr, cu);

  /* Workaround wrong debug info seen on some compilers (particularly,
     RealView 2.2-4.0), where an inlined subroutine is always put at
     the wrong lexical block, like so, for example:

	 1: DW_TAG_subprogram           (low_pc : 0x590, high_pc : 0x5a6)
	   2: DW_TAG_lexical_block      (low_pc : 0x594, high_pc : 0x5a4)
	     3: DW_TAG_lexical_block    (low_pc : 0x598, high_pc : 0x5a2)
	       4: DW_TAG_variable
	         ...
	   2: DW_TAG_inlined_subroutine (low_pc : 0x59c, high_pc : 0x5a0)

     Note that the inlined subroutine is put nested under the
     subprogram.  Also notice the overlapping range between the
     subroutine and the lexical blocks; the inlined subroutine is
     actually inside the innermost lexical block.

     Lookup the latest pending block where BLOCK fits in, and use that
     as superblock for this new block.  */
  if (die->tag == DW_TAG_inlined_subroutine
      && producer_is_realview (cu->producer))
    fixup_block_superblock (block, objfile);
...
read_func_scope (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  block = dwarf2_finish_block (die, cu,
			       new->name, &local_symbols, new->old_blocks,
			       lowpc, highpc, objfile, baseaddr);
...
read_lexical_block_scope (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (local_symbols != NULL || using_directives != NULL)
    dwarf2_finish_block (die, cu,
			 0, &local_symbols,
			 new->old_blocks, new->start_addr,
			 highpc, objfile, baseaddr);
		      ...
		      struct partial_symtab *pst)
{
  ...
  /* At least some GHS tools produce incorrect DW_AT_low_pc and
     DW_AT_high_pc records for compilation units.  Seen with
     "GHS C 2.1 [dual]", but ignore them all for safety.  */
  if (die->tag == DW_TAG_compile_unit)
    {
      attr = dwarf2_attr (die, DW_AT_producer, cu);
      if (attr && ! strncmp (DW_STRING (attr), "GHS", 3))
 	return 0;
    }
...
read_tag_ptr_to_member_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  if (TYPE_CODE (check_typedef (to_type)) == TYPE_CODE_FUNC)
    {
      /* TYPE is the domain of this method, and THIS_TYPE is the type
	   of the method itself (TYPE_CODE_METHOD).  */
      to_type = check_typedef (to_type);
      type = alloc_type (objfile);
      smash_to_method_type (type, domain,
...
read_subroutine_type (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  /* This works around a bug in armcc.  It marks "this" as artificial
     ...
     GCC also has different type info in specifications and
     instantiations; see GCC PR 43053, in which "this" is marked
     as const in the instantiation, but not in the specification.  */

  /* GCC's concrete out-of-line instances of inlined subroutines
     (identifiable as inheriting the type declaration with
     DW_AT_abstract_origin) may end up with a different number of
     arguments as its specification.  For example, artificial
     arguments of constructors may be optimized out (e.g., __in_chrg
     and __vtt_parm).  Since we're interested in the real type of the
     out-of-line subroutine, we limit this workaround to armcc.  We
     could also try still apply the optimization when the
     specification is referenced with DW_AT_specification, and/or
     count the arguments under DIE and check whether it matches the
     number of args in the specification type.  Is it worth it?  */
  if (producer_is_realview (cu->producer))
    {
      ...
      if (spec_die)
	{
	  /* If the specification is a member function, read the type
	     of the class before the type of the member function.
	     This prevents problems; it is safe to be building a
	     member function's type and have an incomplete type for
	     the containing class (in fact, it's typical).  But it is
	     not safe to build a class's type and have an incomplete
	     type for one of the member functions.  We will crash
	     trying to generate a full C++ name, since the argument
	     types are NULL.  */
	  if (spec_die->parent->tag == DW_TAG_structure_type
	      || spec_die->parent->tag == DW_TAG_class_type
	      || spec_die->parent->tag == DW_TAG_union_type)
	    read_type_die (spec_die->parent, spec_cu);
	  ftype = read_type_die (spec_die, spec_cu);
...
/* Concatenate DIRNAME with FILENAME, avoiding duplicate directory
   separators in case DIRNAME ends with a separator.  */

static char *
dwarf_path_concatenate (const char *dirname, const char *filename)
{
			   ...
			   const char *comp_dir)
{
  ...
  if (!IS_ABSOLUTE_PATH (include_name)
      && (dir_name != NULL || comp_dir != NULL))
    {
	 ...
	 DW_AT_name = "./hello.c"  */
      ...
      if (dir_name != NULL)
	{
	  include_name = dwarf_path_concatenate (dir_name, include_name);
	  include_name_to_compare = include_name;
	  make_cleanup (xfree, include_name);
	}
      if (!IS_ABSOLUTE_PATH (include_name) && comp_dir != NULL)
	{
	  include_name_to_compare = dwarf_path_concatenate (comp_dir,
							    include_name);
	}
    }
  ...
  if (!IS_ABSOLUTE_PATH (pst_filename) && pst->dirname != NULL)
    {
      pst_filename = dwarf_path_concatenate (pst->dirname, pst_filename);
    }
...
dwarf2_start_subfile (char *filename, char *dirname, char *comp_dir)
{
  ...
  if (!IS_ABSOLUTE_PATH (filename) && dirname != NULL)
    fullname = dwarf_path_concatenate (dirname, filename);
  else
    fullname = filename;
