	...
	if (method)
	  {
	    ...
	    value = allocate_value (val_type);
	    set_value_address (value, funaddr);
	    struct_return = using_struct_return (exp->gdbarch,
						 value_type (method), value);
	  }
	else if (expect_type != NULL)
	  {
	    struct type *val_type = check_typedef (expect_type);
	    struct value *value = allocate_value (val_type);

	    struct_return = using_struct_return (exp->gdbarch, NULL, value);
	  }
