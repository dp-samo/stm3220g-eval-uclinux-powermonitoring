...
enum type_flag_value
{
  ...
  TYPE_FLAG_NOSIGN = (TYPE_FLAG_UNSIGNED << 1),
  TYPE_FLAG_STUB = (TYPE_FLAG_NOSIGN << 1),
  TYPE_FLAG_TARGET_STUB = (TYPE_FLAG_STUB << 1),
  TYPE_FLAG_STATIC = (TYPE_FLAG_TARGET_STUB << 1),
  TYPE_FLAG_PROTOTYPED = (TYPE_FLAG_STATIC << 1),
  TYPE_FLAG_INCOMPLETE = (TYPE_FLAG_PROTOTYPED << 1),
  TYPE_FLAG_VARARGS = (TYPE_FLAG_INCOMPLETE << 1),
  TYPE_FLAG_VECTOR = (TYPE_FLAG_VARARGS << 1),
  TYPE_FLAG_FIXED_INSTANCE = (TYPE_FLAG_VECTOR << 1),
  TYPE_FLAG_STUB_SUPPORTED = (TYPE_FLAG_FIXED_INSTANCE << 1),
  TYPE_FLAG_NOTTEXT = (TYPE_FLAG_STUB_SUPPORTED << 1),
...
enum type_instance_flag_value
{
  ...
  TYPE_INSTANCE_FLAG_READ_SENSITIVE = (1 << 6)
};
...
/* True if any field of this type is read-sensitive.  Does not apply
   to this type as a whole; see TYPE_READ_SENSITIVE instead.  */

#define TYPE_ANY_FIELD_SENSITIVE(t) (TYPE_MAIN_TYPE (t)->flag_any_field_sensitive)
...
/* If this flag is set on an lvalue's type, then that lvalue
   represents an object which is read-sensitive --- reading or writing
   the value has side effects on the state of the inferior.  GDB
   should avoid accessing such values unless the user has explicitly
   requested that it do so.  (For example, printing a structure
   normally reads the structure's full contents, but if some of its
   members have this flag set, GDB should avoid fetching those
   members.)  */

#define TYPE_READ_SENSITIVE(t)		(TYPE_INSTANCE_FLAGS (t) & TYPE_INSTANCE_FLAG_READ_SENSITIVE)
...
struct main_type
{
  ...
  /* True if any field of this type (not including the type itself)
     is read-sensitive.  */
  unsigned int flag_any_field_sensitive : 1;
...
/* Helper functions to construct a struct or record type.  An
   ...
   Fields are then added using append_composite_type_field*(), which
   returns the newly added field.  A union
