...
#elif defined(PTRACE_GETPROCMAP)
  unsigned long data[6];
  int pid = ptid_get_pid (get_thread_process (current_inferior)->head.id);
...
{
  int pid = lwpid_of (get_thread_lwp (current_inferior));
  int addr = (strcmp (annex, "exec") == 0 ? (int) PTRACE_GETFDPIC_EXEC :
	      (strcmp (annex, "interp") == 0 ? (int) PTRACE_GETFDPIC_INTERP :
	       -1));
