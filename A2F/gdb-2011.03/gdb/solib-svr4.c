...
enable_break (struct svr4_info *info, int from_tty)
{
  ...
  if (sym_addr != 0)
	...
	{
	  /* At least one shared library is loaded, but we could not
	     find the library covering SYM_ADDR.  One way this could
	     happen is if the libraries are not present on the host;
	     another is an incorrect sysroot setting.  Do not fall
	     through, since an already-running binary might have been
	     started with a different interpreter than the one in its
	     .interp section.  */
	  warning (_("Unable to find dynamic linker.\n"
