...
static int
block_physically_contained_in (struct block *a, struct block *b)
{
  if (!a || !b)
    return 0;

  /* Note: we're not handling addrmaps presently, perhaps we
     should.  */
...
/* Check if BLOCK should be a child of the (sibling) blocks read so
   far, but isn't due to bad debug info.  Set the BLOCK's superblock
   if so.  A later finish_block on the BLOCK's parent block skips
   blocks that have a superblock already set.  This assumes bad blocks
   are always read in after the sibling block that should have been
   the parent, and that the best candidate is the last block read in
   where BLOCK fits.  */

void
fixup_block_superblock (struct block *block, struct objfile *objfile)
{
  struct gdbarch *gdbarch = get_objfile_arch (objfile);
  struct pending_block *pblock;
  ...
  for (pblock = pending_blocks; pblock; pblock = pblock->next)
    {
      if (pblock->block == block
	  || BLOCK_FUNCTION (pblock->block) != NULL)
	continue;

      /* Skip blocks we've already determined to be lexically nested
	 within BLOCK.  Those will also be physically contained in
	 BLOCK, but we're only looking for siblings or parents, not
	 children.  */
      if (contained_in (pblock->block, block))
	continue;

      if (block_physically_contained_in (block, pblock->block))
	{
...
start_subfile (char *name, char *dirname)
{
  ...
  for (subfile = subfiles; subfile; subfile = subfile->next)
	...
	subfile_name = subfile->name;
      ...
      if (gdb_filename_cmp (subfile->name, name) == 0)
	{
