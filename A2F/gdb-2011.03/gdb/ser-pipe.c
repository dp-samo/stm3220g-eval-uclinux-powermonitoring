...
pipe_open (struct serial *scb, const char *name)
{
  ...
  return -1;
#else
  struct pipe_state *state, *err_state;
  /* This chunk: */
  ...
  if (err_pdes[1] != -1)
    close (err_pdes[1]);
  /* :end chunk */
  state = XZALLOC (struct pipe_state);
  state->pid = pid;
  scb->fd = pdes[0];
  scb->state = state;

  err_state = XZALLOC (struct pipe_state);
  err_state->is_stderr = 1;
  scb->error_scb = serial_for_ops ("pipe's stderr", scb->ops);
  scb->error_scb->state = err_state;
  scb->error_scb->fd = err_pdes[0];
