...
static int micromips_instruction_has_delay_slot (struct gdbarch *, CORE_ADDR,
						 int);
static int mips16_instruction_has_delay_slot (struct gdbarch *,
					      CORE_ADDR, int);
...
/* macro/2008-01-29: Some MIPS16 functions have a call stub prepended
   ...
   macro/2009-10-30: This seems to happen for microMIPS functions too,
   even though they do not use call stubs, hmm...  */
			  ...
			  struct reggroup *reggroup)
{
  ...
  /* ???raw_p is always true  */
  raw_p = rawnum < gdbarch_num_regs (gdbarch);

  if (gdbarch_register_name (gdbarch, regnum) == NULL
      || gdbarch_register_name (gdbarch, regnum)[0] == '\0')
    return 0;
  if (float_p && MIPS_FPU_TYPE (gdbarch) == MIPS_FPU_NONE)
  ...
  /* Registers that need to be saved/restored in order to push or pop
     ...
     values from a saved register cache also uses pseudo registers.
     We only save and restore the user-level general and floating
  ...
  if (reggroup == save_reggroup || reggroup == restore_reggroup)
    return pseudo && raw_p
	   && (general_p || float_p || rawnum == mips_regnum (gdbarch)->pc
...
build_builtin_type_mips_double_float_reg (struct gdbarch *gdbarch)
{
  ...
  if (gdbarch_tdep (gdbarch)->double_float_reg_type == NULL)
    {
      struct type *t;
      struct type *float_type = builtin_type (gdbarch)->builtin_float;
      struct type *double_type = builtin_type (gdbarch)->builtin_double;
      struct type *int32_type = builtin_type (gdbarch)->builtin_int32;
      struct type *int64_type = builtin_type (gdbarch)->builtin_int64;

      t = arch_composite_type (gdbarch,
			       "__gdb_builtin_type_mips_double_float_reg",
			       TYPE_CODE_UNION);
      TYPE_NAME (t) = "builtin_type_mips_double_float_reg";
      append_composite_type_field (t, "d", double_type);
      append_composite_type_field (t, "l", int64_type);
      append_composite_type_field (t, "s", init_vector_type (float_type, 2));
      append_composite_type_field (t, "w", init_vector_type (int32_type, 2));
      gdbarch_tdep (gdbarch)->double_float_reg_type = t;
    }
  return gdbarch_tdep (gdbarch)->double_float_reg_type;
}
...
build_builtin_type_mips_single_float_reg (struct gdbarch *gdbarch)
{
  ...
  if (gdbarch_tdep (gdbarch)->single_float_reg_type == NULL)
    {
      struct type *float_type = builtin_type (gdbarch)->builtin_float;
      struct type *int32_type = builtin_type (gdbarch)->builtin_int32;
      struct type *t;
      ...
      t = arch_composite_type (gdbarch,
			       "__gdb_builtin_type_mips_single_float_reg",
			       TYPE_CODE_UNION);
      TYPE_NAME (t) = "builtin_type_mips_single_float_reg";
      append_composite_type_field (t, "s", float_type);
      append_composite_type_field (t, "w", int32_type);
      gdbarch_tdep (gdbarch)->single_float_reg_type = t;
    }
  return gdbarch_tdep (gdbarch)->single_float_reg_type;
}
...
mips_register_type (struct gdbarch *gdbarch, int regnum)
{
  ...
  if (mips_float_register_p (gdbarch, regnum))
      ...
      if (regnum < gdbarch_num_regs (gdbarch))
	{
	  ...
	  if (mips_float_regsize (gdbarch, NULL) == 8)
      ...
      else
	{
	  ...
	  if (mips_float_regsize (gdbarch, NULL) == 8
	      || ((regnum - gdbarch_num_regs (gdbarch)) & 1) == 0)
	    return build_builtin_type_mips_double_float_reg (gdbarch);
	  else
	    return build_builtin_type_mips_single_float_reg (gdbarch);
	}
...
/* Tell if the program counter value in MEMADDR is in a MIPS32 function.  */
...
/* 16-bit/32-bit-high-part instruction formats.  */
...
/* Decode a 3-bit register argument -- 0 and 1 map to $16 and $17,
   and 2 - 7 map to $2 - $7 respectively.  */
static int
mips_decode_reg3 (int reg)
{
  return ((reg + 0xe) & 0xf) + 2;
}
...
micromips_next_pc (struct frame_info *frame, CORE_ADDR pc)
{
  ...
  switch (mips_insn_size (ISA_MICROMIPS, insn))
	...
	case 0x23: /* BEQZ16: bits 100011 */
	  {
	    int rs = mips_decode_reg3 (b7s3_reg (insn));
			 ...
			 unsigned int extension, unsigned int insn)
{
  ...
  switch (op)
	...
	reg = get_frame_register_signed (frame, mips_decode_reg3 (upk.regx));
	if (reg == 0)
	  pc += (upk.offset << 1) + 2;
	else
	  pc += 2;
...
mips_about_to_return (struct gdbarch *gdbarch, CORE_ADDR pc)
{
  ...
  /* This used to check for MIPS16, but this piece of code is never
     called for MIPS16 functions.  Likewise microMIPS.  */
...
#ifdef MIPSSDE
  if (pc_in_unmapped_range (pc, osect = find_pc_overlay (pc)))
    /* Use unmapped section base address.  */
    sec_addr = osect->the_bfd_section->lma;
  else if ((osect = find_pc_section (pc)) != NULL)
    /* Use mapped section base address.  */
    sec_addr = obj_section_addr (osect);
  else
			  ...
			  gdb_byte *readbuf, const gdb_byte *writebuf)
{
  struct gdbarch_tdep *tdep = gdbarch_tdep (gdbarch);
  struct type *type = value_type (function);
  int mips16 = mips_pc_is_mips16 (gdbarch, value_address (function));
  ...
  if (TYPE_LENGTH (type) > 2 * MIPS64_REGSIZE)
	...
	{
	  /* A single or double floating-point value that fits in GPR2.  */
	  if (mips_debug)
			  ...
			  int struct_return, CORE_ADDR struct_addr)
{
  ...
  for (argnum = 0; argnum < nargs; argnum++)
	  ...
	  && float_argreg <= MIPS_LAST_FP_ARG_REGNUM (gdbarch))
	{
	  ...
	  else if (mips_float_regsize (gdbarch, NULL) == 8 && len == 8)
	    {
	      ...
	      /* Full doubleword.  */
	      regval = extract_unsigned_integer (val, 8, byte_order);
	      if (mips_debug)
		fprintf_unfiltered (gdb_stdlog, " - fpreg=%d val=%s",
		       ...
		       gdb_byte *readbuf, const gdb_byte *writebuf)
{
  ...
  if (TYPE_CODE (type) == TYPE_CODE_STRUCT
      ...
      if (!mips16)
	{
      ...
      else if (MIPS32_REGSIZE >= mips_float_regsize (gdbarch, NULL))
	{
...
{				/* do values for FP (float) regs */
  ...
  if (mips_float_regsize (gdbarch, frame) == 8
      || (regnum - gdbarch_num_regs (gdbarch)) % 2 == 0)
    {
		     ...
		     int regnum)
{
  ...
  if (!frame_register_read (frame, regnum, raw_buffer)
      || (regnum >= gdbarch_num_regs (gdbarch)
	  && regcache_valid_p (get_current_regcache (),
			       regnum - gdbarch_num_regs (gdbarch)) < 0))
    {
      fprintf_filtered (file, "%s: [Invalid]",
			gdbarch_register_name (gdbarch, regnum));
      return;
		      ...
		      struct frame_info *frame, const char *args)
{
  ...
  if (type != MIPS_FPU_NONE)
    {
      regcache_cooked_read_unsigned (get_current_regcache (),
				     mips_regnum (gdbarch)->fp_control_status,
				     &fsr);
      if (regcache_valid_p (get_current_regcache (),
			    mips_regnum (gdbarch)->fp_control_status) < 0)
	type = MIPS_FPU_NONE;
    }
  ...
  fprintf_filtered (file, "fpu type: %s\n",
		    ...
		    : type == MIPS_FPU_64 ? "64-bit registers, double-precision"
		    : "none / unused");
  ...
  fprintf_filtered (file, "reg size: %d bits\n",
		    mips_float_regsize (gdbarch, frame) * 8);
  				...
  				struct frame_info *frame)
{
  ...
  return (breakpoint_here_p (get_frame_address_space (frame), pc + size)
	  != no_breakpoint_here);
}
...
static int
mips16_instruction_has_delay_slot (struct gdbarch *gdbarch,
				   CORE_ADDR addr, int mustbe32)
{
  unsigned short inst;
  int status;
...
mips_adjust_breakpoint_address (struct gdbarch *gdbarch, CORE_ADDR bpaddr)
{
  ...
  if (find_pc_partial_function (bpaddr, NULL, &func_addr, NULL)
    ...
    boundary = unmake_compact_addr (func_addr);
  ...
  if (mips_pc_is_mips (bpaddr))
  ...
  else
    {
      ...
      /* The only MIPS16 instructions with delay slots are jal, jalr
         ...
         MicroMIPS has a whole range of jumps and branches with delay
         slots, some of which take 4 bytes and some take 2 bytes, so
         the idea is the same.
         XXX We have to assume that bpaddr is not the second half of
         an extended instruction.  */
      instruction_has_delay_slot
	= (mips_pc_is_micromips (gdbarch, bpaddr)
  ...
  if (strncmp (name, mips_str_mips16_call_stub,
	       ...
	       || name[strlen (mips_str_mips16_call_stub)] == 'd')
	{
	  if (pc == start_addr)
		...
		{
		  ULONGEST inst = mips_fetch_instruction (gdbarch, ISA_MIPS,
							  target_pc, NULL);
		  CORE_ADDR addr = inst;
...
static int
mips_in_return_stub (struct gdbarch *gdbarch, CORE_ADDR pc, char *name)
{
  CORE_ADDR start_addr;
...
/* This is the version we used before pushing the non-pic work
   upstream.  We keep it for backwards compatibility with older
   binaries.  */
static CORE_ADDR
mips_skip_pic_trampoline_code_old (struct frame_info *frame, CORE_ADDR pc)
{
  struct gdbarch *gdbarch = get_frame_arch (frame);
  enum bfd_endian byte_order = gdbarch_byte_order (gdbarch);
  struct obj_section *s;
  ...
  if (target_read_memory (pc, stub_code, 16) != 0)
    return 0;
  for (i = 0; i < 4; i++)
    stub_words[i] = extract_unsigned_integer (stub_code + i * 4, 4, byte_order);
...
mips_skip_trampoline_code (struct frame_info *frame, CORE_ADDR pc)
{
  ...
  target_pc = mips_skip_pic_trampoline_code_old (frame, pc);
  if (target_pc)
    return target_pc;
...
static int
mips_register_rtos_regno (struct gdbarch *gdbarch, int regnum)
{
  if (regnum <= MIPS_RA_REGNUM)
    return regnum;

  if (regnum == mips_regnum (gdbarch)->pc)
    return RTOS_MIPS32_REG_PC;

  if (regnum >= mips_regnum (gdbarch)->fp0
      && regnum < mips_regnum (gdbarch)->fp0 + 32)
    return RTOS_MIPS32_REG_F0 + (regnum - mips_regnum (gdbarch)->fp0);
  if (regnum == mips_regnum (gdbarch)->fp_control_status)
    return RTOS_MIPS32_REG_FSR;
  if (regnum == mips_regnum (gdbarch)->fp_implementation_revision)
    return RTOS_MIPS32_REG_FIR;

  if (regnum == mips_regnum (gdbarch)->badvaddr)
    return RTOS_MIPS32_REG_BAD;
  if (regnum == mips_regnum (gdbarch)->cause)
    return RTOS_MIPS32_REG_CAUSE;

  if (regnum == mips_regnum (gdbarch)->hi)
    return RTOS_MIPS32_REG_HI;
  if (regnum == mips_regnum (gdbarch)->lo)
    return RTOS_MIPS32_REG_LO;
...
mips_register_g_packet_guesses (struct gdbarch *gdbarch)
{
  ...
  /* The CodeSourcery MIPS sprite typically returns 72 registers;
     GPRs, FPRs, but not CP0 or CP2.  */
  register_remote_g_packet_guess (gdbarch, 72 * 4, mips_tdesc_gp32);
  register_remote_g_packet_guess (gdbarch, 72 * 8, mips_tdesc_gp64);
...
mips_gdbarch_init (struct gdbarch_info info, struct gdbarch_list *arches)
{
  ...
  if (tdesc_has_registers (info.target_desc))
      ...
      if (mips_regnum.dspacc >= 0)
	{
	  ...
	  if (feature == NULL)
	    {
	      /* The DSP registers are optional; record that they are
		 absent.  */
	      mips_regnum.dspacc = -1;
	      mips_regnum.dspctl = -1;
	    }
 ...
 */
  ...
  set_gdbarch_register_rtos_regno (gdbarch, mips_register_rtos_regno);
...
#ifdef MIPSSDE
  mips_mdebug_append_sniffers (gdbarch);
#endif
...
mips_dump_tdep (struct gdbarch *gdbarch, struct ui_file *file)
{
  ...
  fprintf_unfiltered (file,
		      ...
		      (MIPS_FPU_TYPE (gdbarch) == MIPS_FPU_NONE ? "none"
		       ...
		       : MIPS_FPU_TYPE (gdbarch) == MIPS_FPU_64 ? "64-bit"
		       : "???"));
}
...
_initialize_mips_tdep (void)
{
  ...
  observer_attach_new_objfile (mips_new_objfile);
#endif
...
This option selects the compressed ISA encoding used in functions for which\n\
there is no symbol information available.  The encoding can be set to either\n\
of:\n\
  mips16\n\
  micromips\n\
