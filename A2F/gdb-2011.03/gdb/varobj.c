...
varobj_set_value (struct varobj *var, char *expression)
{
  ...
  /* The value of a changeable variable object must not be lazy.
     Except if it's frozen, in which case we might have never
     requested for the value to be read.  */
  gdb_assert (var->frozen || !value_lazy (var->value));

  /* But a frozen value that is never read can happen only
     for read sensitive values -- which automatically become
     frozen when created.  */
  gdb_assert (!value_lazy (var->value) || TYPE_READ_SENSITIVE (var->type));
  ...
  /* Unless the value is read-sensitive, force it to be read back in
     and installed as the current value of the varobj. So, if we write
     to a an I/O area, -var-assign will report the value as reads back,
     not as written -- which may differ if some bits are masked off,
     or something.  */

  if (var->type == NULL || !TYPE_READ_SENSITIVE (var->type))
    {
      /* Make sure the value is read in.  */
      set_value_lazy (val, 1);
      if (value_bitsize (val))
	{
	  set_value_lazy (value_parent (val), 1);
	}

      /* We surely want to store new value inside varobj, in the same of
	 consistency and also so that -var-assign can report the new value.
	 However, if we just store the new value, then next -var-update will
	 not report varobj as changed, even though the value did change
	 since previous -var-update. To workaround this, set var->updated,
	 which will be noticed by -var-update.

	 If a variable had a value of '1', we've set it to '333' and then
	 set again to '1', when -var-update will report this variable as
	 changed. There's no need to optimize that, because return
	 value of -var-update should be considered an approximation.  */
      var->updated = install_new_value (var, val, 0 /* Compare values. */);
    }
  else
    {
      /* Read sensitive value, so we're not allowed to read it.  Mark it lazy
	 to indicate that we don't have up-to-date value.  */
      set_value_lazy (var->value, 1);
      var->not_fetched = 1;
      /* Please frontends that ignore return from -var-assign.  */
      var->updated = 1;
    }
  ...
  if (!initial && changeable)
    {
	 ...
	 -var-update. So need to mark the varobj as changed.  */
      if (var->updated)
	{
	  changed = 1;
	}
	  ...
	  if (c != NULL)
	    {
	      ...
	      if (!c->frozen)
		{
		  /* Ordinary child.  Process it later.  */
		  VEC_safe_push (varobj_update_result, stack, &r);
		}
	      else if (c->updated)
		{
		  /* Frozen child that was previously set via
		     -var-assign.  Report it as changed.  There's no
		     need to recurse since only leaf varobjs can be
		     assigned to anyway.  */
		  VEC_safe_push (varobj_update_result, result, &r);
		  c->updated = 0;
		}
			 ...
			 struct value *value)
{
  ...
  if (child->type && TYPE_READ_SENSITIVE (child->type))
    child->frozen = 1;
