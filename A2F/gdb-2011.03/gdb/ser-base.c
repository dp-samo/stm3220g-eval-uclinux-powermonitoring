...
static void
read_and_print (struct serial *scb)
{ 
    ssize_t s;
    char buf[81];
	...
	int num_bytes = -1;
	if (scb->ops->avail)
	  num_bytes = scb->ops->avail (scb, scb->fd);
	if (num_bytes != -1)
  ...
  while (1)
    {
      ...
      if (scb->error_scb)
	{
	  FD_SET (scb->error_scb->fd, &readfds);
	  maxfd = scb->error_scb->fd > maxfd ? scb->error_scb->fd : maxfd;
	}

      if (timeout >= 0)
	numfds = gdb_select (maxfd + 1, &readfds, 0, &exceptfds, &tv);
      else
	numfds = gdb_select (maxfd + 1, &readfds, 0, &exceptfds, 0);
      ...
      if (numfds <= 0)
	{
      ...
      if (scb->error_scb)
	{
	  if (FD_ISSET (scb->error_scb->fd, &readfds))
	    {
	      read_and_print (scb->error_scb);
	      if (!FD_ISSET (scb->fd, &readfds) 
		  && !FD_ISSET (scb->fd, &exceptfds))
		continue;
	    }	  
	}
