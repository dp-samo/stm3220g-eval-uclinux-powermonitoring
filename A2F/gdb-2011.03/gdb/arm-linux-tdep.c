...
static void
arm_uclinux_init_abi (struct gdbarch_info info,
		      struct gdbarch *gdbarch)
{
  arm_linux_init_abi (info, gdbarch);

  /* Do not use the SVR4 solib-ops, which will relocate the executable
     to zero.  */
...
_initialize_arm_linux_tdep (void)
{
  gdbarch_register_osabi (bfd_arch_arm, 0, GDB_OSABI_LINUX,
			  arm_linux_init_abi);
  gdbarch_register_osabi (bfd_arch_arm, 0, GDB_OSABI_UCLINUX,
			  arm_uclinux_init_abi);
}
