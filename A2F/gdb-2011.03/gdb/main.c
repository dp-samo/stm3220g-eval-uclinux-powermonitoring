...
get_python_path (void)
{
  ...
  for (start = paths; *start != '\0'; start = end)
    {
      char *path, *rpath;
      /* Deliberately use ':' here and DIRNAME_SEPARATOR below.  The
	 input path must not contain a drive letter.  For local use
	 this is sufficient; and passing the semicolon through
	 configury could be tricky.  */
      end = (const char *) strchr (start, ':');
      if (end == NULL)
	end = start + strlen (start);
      path = savestring (start, end - start);
      rpath = make_relative_prefix (gdb_program_name, BINDIR, path);
      if (rpath == NULL)
	rpath = path;
      else
	xfree (path);
      ...
      if (relocated == NULL)
	relocated = xstrdup (rpath);
      else
	{
	  char *tmp = xstrprintf ("%s%c%s", relocated, DIRNAME_SEPARATOR, rpath);
	  xfree (relocated);
	  relocated = tmp;
	}
...
captured_main (void *data)
{
  ...
  /* Parse arguments and options.  */
  {
    ...
    static const struct option long_options[] =
    {
      ...
      {"only-system-gdbinit", no_argument, &only_system_gdbinit, 1},
...
Excess command line arguments ignored. (%s%s)\n"),
  ...
  if (home_gdbinit && !inhibit_gdbinit && !only_system_gdbinit)
...
Can't attach to process and specify a core file at the same time."));
  ...
  if (local_gdbinit && !inhibit_gdbinit && !only_system_gdbinit)
