...
mips_mdebug_frame_cache (struct frame_info *this_frame, void **this_cache)
{
  CORE_ADDR pc;
  ...
  gdb_assert (!in_prologue (gdbarch, get_frame_pc (this_frame),
     ...
     saved.  */
  {
	...
	{
	  cache->saved_regs[gdbarch_num_regs (gdbarch) + ireg].addr
	    = reg_position;
     ...
     saved.  */
  {
    ...
    cache->saved_regs[gdbarch_num_regs (gdbarch)
      ...
      = cache->saved_regs[gdbarch_num_regs (gdbarch) + MIPS_RA_REGNUM];
  }
...
static void
mips_mdebug_frame_this_id (struct frame_info *this_frame, void **this_cache,
			   struct frame_id *this_id)
{
  struct mips_frame_cache *info = mips_mdebug_frame_cache (this_frame,
							   this_cache);
  (*this_id) = frame_id_build (info->base, get_frame_func (this_frame));
}

static struct value *
mips_mdebug_frame_prev_register (struct frame_info *this_frame,
...
static int
mips_mdebug_frame_sniffer (const struct frame_unwind *self,
			   struct frame_info *this_frame, void **this_cache)
{
  ...
  /* Don't use this but on standard MIPS.  */
  ...
  if (pc > PROC_LOW_ADDR (proc_desc)
      && !in_prologue (get_frame_arch (this_frame), pc,
		       PROC_LOW_ADDR (proc_desc)))
    return 1;
...
static CORE_ADDR
mips_mdebug_frame_base_address (struct frame_info *this_frame,
				void **this_cache)
{
  struct mips_frame_cache *info = mips_mdebug_frame_cache (this_frame,
							   this_cache);
...
mips_mdebug_frame_base_sniffer (struct frame_info *this_frame)
{
  if (mips_mdebug_frame_sniffer (&mips_mdebug_frame_unwind, this_frame, NULL))
...
{
  frame_unwind_append_unwinder (gdbarch, &mips_mdebug_frame_unwind);
