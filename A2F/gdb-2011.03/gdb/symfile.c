...
generic_load (char *args, int from_tty)
{
  ...
  entry = gdbarch_addr_bits_remove (target_gdbarch,
...
/* Function: section_is_real_overlay (SECTION)
   Returns true if SECTION is *really* an overlay, not a fake
   shadow mapping as set up by MIPS to handle cached/uncached
   mapping. */

int
section_is_real_overlay (struct obj_section *section)
{
                          ...
                          bfd_byte *buf)
{
  ...
  /* ???: Assume executables and shared libraries have been relocated.
     This is needed for uClinux targets where executables are linked with
     --emit-relocs.  */
