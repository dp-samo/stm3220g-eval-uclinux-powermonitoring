...
static char *gdb_sysroot_on_target;
...
solib_find (char *in_pathname, int *fd)
{
  ...
  if (gdb_sysroot_on_target != NULL && *gdb_sysroot_on_target != 0)
    {
      int prefix_len = strlen (gdb_sysroot_on_target);

      /* Don't include trailing slashes -- after we strip this
	 prefix from gdb_sysroot_on_target, we want it to still
	 be an absolute path.  */
      while (prefix_len > 0
	     && IS_DIR_SEPARATOR (gdb_sysroot_on_target[prefix_len - 1]))
	prefix_len--;

      /* Should we try to normalize paths here? */
      if (strncmp (gdb_sysroot_on_target, in_pathname, prefix_len) == 0)
	in_pathname += prefix_len;
    }
...
`set solib-search-path'."),
  ...
  add_setshow_filename_cmd ("sysroot-on-target", class_support,
			    &gdb_sysroot_on_target, _("\
Set the location of system root on target."), _("\
Show the location of system root on target."), _("\
The location of system root on target is a prefix that will be removed from\n\
the names of all shared libraries in the program befor GDB attempts to locate\n\
those shared libraries on the host system."),
			    reload_shared_libraries,
			    NULL,
			    &setlist, &showlist);
