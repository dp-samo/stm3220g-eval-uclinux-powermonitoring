...
struct dwarf2_frame_state
{
  ...
  /* Negated DW_CFA_offset records as seen with some GHS tools.  */
  int ghs_reg_offsets_negated;

  /* Invalid DW_CFA_same_value default for the stack pointer as seen
     with some GHS tools.  */
  int ghs_sp_same_value;
};
			  ...
			  struct dwarf2_fde *fde)
{
  ...
  /* GHS tools quirks -- seen with "GHS C 6.0 [dual]" and may require
     more accurate matching if further information is available.  */
  if (s->producer != NULL && strncmp (s->producer, "GHS", 3) == 0)
    {
      /* At least some versions produce negated DW_CFA_offset records.  */
      fs->ghs_reg_offsets_negated = 1;

      /* At least some versions produce a DW_CFA_same_value initial
         record for the stack pointer, but fail to update it when the
         register is modified to build a stack frame.  */
      fs->ghs_sp_same_value = 1;
    }
}

