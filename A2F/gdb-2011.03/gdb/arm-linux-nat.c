...
store_fpregister (const struct regcache *regcache, int regno)
{
  ...
  /* Store fpsr.  */
  if (ARM_FPS_REGNUM == regno
      && regcache_valid_p (regcache, ARM_FPS_REGNUM) > 0)
...
store_fpregs (const struct regcache *regcache)
{
  ...
  /* Store fpsr.  */
  if (regcache_valid_p (regcache, ARM_FPS_REGNUM) > 0)
    regcache_raw_collect (regcache, ARM_FPS_REGNUM, fp + NWFPE_FPSR_OFFSET);
...
store_regs (const struct regcache *regcache)
{
  ...
  if (arm_apcs_32 && regcache_valid_p (regcache, ARM_PS_REGNUM) > 0)
...
store_wmmx_regs (const struct regcache *regcache)
{
  ...
  for (regno = 0; regno < 16; regno++)
    if (regcache_valid_p (regcache, regno + ARM_WR0_REGNUM) > 0)
      regcache_raw_collect (regcache, regno + ARM_WR0_REGNUM,
			    &regbuf[regno * 8]);

  for (regno = 0; regno < 2; regno++)
    if (regcache_valid_p (regcache, regno + ARM_WCSSF_REGNUM) > 0)
      regcache_raw_collect (regcache, regno + ARM_WCSSF_REGNUM,
			    &regbuf[16 * 8 + regno * 4]);

  for (regno = 0; regno < 4; regno++)
    if (regcache_valid_p (regcache, regno + ARM_WCGR0_REGNUM) > 0)
