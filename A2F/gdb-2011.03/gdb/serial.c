...
static void serial_free (struct serial *scb);
...
struct serial *
serial_for_ops (const char *name, struct serial_ops *ops)
{
  struct serial *scb = XCALLOC (1, struct serial);
...
/* This is bogus.  It's not our fault if you pass us a bad scb...!  Rob, you
  ...
  if (scb->error_scb)
    do_serial_close (scb->error_scb, really_close);
