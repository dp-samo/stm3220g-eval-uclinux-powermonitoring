  ...
  if (count > 0)
      ...
      if (!single_inst)
	{
	  ...
	  if (!skip_subroutines && !single_inst
		...
		{
		  /* If stepping into an inline frame didn't appear to
		     make progress, then keep stepping.  This may be
		     needed when inline call site debug info is
		     missing (e.g., no DW_AT_call_line/DW_AT_call_file
		     support), because we use the current PC/line as
		     fallback guess.  See find_frame_sal.  */
		  sal = find_pc_line (get_frame_pc (frame), 0);
		  if (tp->current_symtab == sal.symtab
...
static void
print_return_value (struct type *func_type, struct value *function)
{
  struct gdbarch *gdbarch = get_regcache_arch (stop_registers);
  struct type *type = value_type (function);
  struct cleanup *old_chain;
  struct ui_stream *stb;
  struct value *function_value, *value;

  CHECK_TYPEDEF (type);
  gdb_assert (TYPE_CODE (type) != TYPE_CODE_VOID);

  function_value = allocate_value (type);
  set_value_address (function_value, value_address (function));
  ...
  switch (gdbarch_return_value (gdbarch, func_type, function_value,
  				NULL, NULL, NULL))
    {
    ...
    case RETURN_VALUE_ABI_PRESERVES_ADDRESS:
      ...
      set_value_address (value, value_address (function));
      gdbarch_return_value (gdbarch, func_type, function_value, stop_registers,
	  ...
	  TRY_CATCH (ex, RETURN_MASK_ALL)
	    {
		 ...
		 delete the breakpoint.  */
	      ...
	      set_value_address (value,
				 BLOCK_START (SYMBOL_BLOCK_VALUE (a->function)));
	      print_return_value (SYMBOL_TYPE (a->function), value);
	    }
	  if (ex.reason < 0)
