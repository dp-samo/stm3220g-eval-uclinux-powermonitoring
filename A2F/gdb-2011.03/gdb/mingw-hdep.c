...
/* Wrapper for isatty.  The argument to this function is the same as
   the traditional argument to isatty on POSIX platforms.  */

static HANDLE hstdin = INVALID_HANDLE_VALUE;
static HANDLE hstdout = INVALID_HANDLE_VALUE;
static HANDLE hstderr = INVALID_HANDLE_VALUE;
static int saved_handles = 0;

static void
save_std_handles (void)
{
  /* Get handles early.  If something later on redirects
     std{in,out,err} to some other pipes, we don't want to mistake
     those for a Cygwin TTY.  Cache pipe-ness now, since calling
     GetFileType can apparently hang.  */
  ...
  if (GetFileType (hstderr) != FILE_TYPE_PIPE)
...
gdb_isatty (int fd)
{
  ...
  /* If we're running GDB under a cygwin ssh session, or under a
     "CYGWIN=tty" shell, stdin{in,out,err} will be seen as native
     Windows PIPEs.  A frontend invoking GDB as a child will typically
     also communicate with GDB using Windows pipes.  As an heuristic,
     assume a tty only on the CLI interpreter.  */
  h = (HANDLE) _get_osfhandle (fd);

  if ((h == hstdin
       || h == hstdout
       || h == hstderr)
      /* Can not use current_interp_named_p, because the interpreter
	 may not be initialized yet.  */
      && strcmp (interpreter_p, INTERP_CONSOLE) == 0)
    return 1;
