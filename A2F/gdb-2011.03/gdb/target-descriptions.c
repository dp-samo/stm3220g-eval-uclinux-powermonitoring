...
{
  struct tdesc_space *space = value_computed_closure (v);
  unsigned bit_length = TYPE_LENGTH (value_type (v)) * 8, real_length;
  ...
  /* Adjust to always write a 32-bit and aligned quantity.  We do extra
     reads from the target in this case - bad for read sensitive registers.
     Need to find a cache of the read value higher up in GDB for bitfields
     and char members.  Plus the magical 32-bit transfer size should not be
     hard coded (maybe it should come from the XML, and be translated into
     a container type).  FIXME!  We could read/write value_parent instead,
     but it could be too big.  */
  if (value_bitsize (v))
    bit_length = value_bitsize (v);

  real_start = offset;
  real_length = bit_length;
  if (real_start % 4 != 0)
    {
      real_length += 8 * (real_start % 4);
      real_start = real_start - (real_start % 4);
    }
  if (real_length > 32)
  ...
  if (bit_length != 32)
      ...
      if (value_bitsize (v))
	{
	  ...
	  modify_field (value_type (v), buf, fieldval,
			value_bitpos (v) + 8 * (offset - real_start),
			bit_length);
	}
      else
	memcpy (buf, value_contents_all_raw (fromval), bit_length / 8);
    }
  else
    memcpy (buf, value_contents_all_raw (fromval), 4);
