...
/* Allocate the hash table used by copy_type_recursive to walk
   types without duplicates.  If OBJFILE != NULL, we use its obstack,
   because OBJFILE is about to be deleted.  */

htab_t
create_copied_types_hash (struct objfile *objfile)
{
  if (objfile == NULL)
    return htab_create_alloc (1, type_pair_hash, type_pair_eq,
			      xfree, xcalloc, xfree);
...
/* Recursively copy (deep copy) TYPE, if it is associated with
   OBJFILE.  OBJFILE may be NULL to copy types associated with no
   objfile.  Return a new type allocated using malloc, a saved type if
   we have already visited TYPE (using COPIED_TYPES), or TYPE if it
		     ...
		     htab_t copied_types)
{
  ...
  if (objfile != NULL && ! TYPE_OBJFILE_OWNED (type))
    return type;

  /* This type shouldn't be pointing to any types in other objfiles;
     if it did, the type might disappear unexpectedly.  This check is
     correct even if OBJFILE == NULL; in that case we do not expect to
     find types from any object file.  */
  ...
  if (objfile == NULL)
    stored = XCALLOC (1, struct type_pair);
  else
				     ...
				     struct type *field, int alignment)
{
  ...
  if (TYPE_READ_SENSITIVE (field) || TYPE_ANY_FIELD_SENSITIVE (field))
    TYPE_ANY_FIELD_SENSITIVE (t) = 1;
