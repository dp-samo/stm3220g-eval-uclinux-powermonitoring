...
fill_gprs64 (const struct regcache *regcache, uint64_t *vals)
{
  ...
  for (regno = 0; regno < ppc_num_gprs; regno++)
    if (regcache_valid_p (regcache, tdep->ppc_gp0_regnum + regno) > 0)
...
fill_fprs (const struct regcache *regcache, double *vals)
{
  ...
  for (regno = tdep->ppc_fp0_regnum;
       regno < tdep->ppc_fp0_regnum + ppc_num_fprs;
       regno++)
    if (regcache_valid_p (regcache, regno) > 0)
	     ...
	     uint32_t *fpscr)
{
  ...
  if (regcache_valid_p (regcache, gdbarch_pc_regnum (gdbarch)) > 0)
    regcache_raw_collect (regcache, gdbarch_pc_regnum (gdbarch), iar);
  if (regcache_valid_p (regcache, tdep->ppc_ps_regnum) > 0)
    regcache_raw_collect (regcache, tdep->ppc_ps_regnum, msr);
  if (regcache_valid_p (regcache, tdep->ppc_cr_regnum) > 0)
    regcache_raw_collect (regcache, tdep->ppc_cr_regnum, cr);
  if (regcache_valid_p (regcache, tdep->ppc_lr_regnum) > 0)
    regcache_raw_collect (regcache, tdep->ppc_lr_regnum, lr);
  if (regcache_valid_p (regcache, tdep->ppc_ctr_regnum) > 0)
    regcache_raw_collect (regcache, tdep->ppc_ctr_regnum, ctr);
  if (regcache_valid_p (regcache, tdep->ppc_xer_regnum) > 0)
    regcache_raw_collect (regcache, tdep->ppc_xer_regnum, xer);
  if (tdep->ppc_fpscr_regnum >= 0
      && regcache_valid_p (regcache, tdep->ppc_fpscr_regnum) > 0)
...
store_regs_user_thread (const struct regcache *regcache, pthdb_pthread_t pdtid)
{
  ...
  for (i = 0; i < ppc_num_gprs; i++)
    if (regcache_valid_p (regcache, tdep->ppc_gp0_regnum + i) > 0)
      {
	if (arch64)
	  {
			  ...
			  pthdb_tid_t tid)
{
  ...
  if (regno == -1 || special_register_p (gdbarch, regno))
      ...
      else
	{
	  ...
	  if (tdep->ppc_mq_regnum >= 0)
	    if (regcache_valid_p (regcache, tdep->ppc_mq_regnum) > 0)
