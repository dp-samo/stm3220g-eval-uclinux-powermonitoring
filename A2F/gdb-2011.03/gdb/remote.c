...
struct remote_state
{
  ...
  /* True if the stub requires a successful authentication before connecting.  */
  int must_auth;
		     ...
		     const gdb_byte *writebuf, ULONGEST offset, LONGEST len)
{
  ...
  if (writebuf != NULL)
	...
	case TARGET_OBJECT_SPACES:
	  return remote_write_qxfer (ops, "spaces", annex, writebuf, 
				     offset, len,
				     &remote_protocol_packets[PACKET_qXfer_spaces]);
