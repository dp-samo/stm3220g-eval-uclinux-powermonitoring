...
/* A Python function which returns whether the current target is
   big-endian.  Currently CodeSourcery local; for upstream, maybe we
   should expose the entire gdbarch.  */

static PyObject *
gdbpy_is_big_endian (PyObject *self, PyObject *args)
{
  PyObject *result;
...
Enables or disables printing of Python stack traces."),
  ...
  /* CSL LOCAL */
  /* We can not set sys.path until after Py_Initialize, which will complain
     to stderr if it can not find libraries.  */
  Py_FrozenFlag = 1;
  /* Similarly, we will not be able to find 'import site'.  */
  Py_NoSiteFlag = 1;
  ...
  /* CSL LOCAL */
  {
    extern char *get_python_path (void);
    char *paths = get_python_path ();
    PyObject *site;
    if (paths != NULL)
      {
	PySys_SetPath (paths);
    ...
    /* We suppressed 'import site' above so do it now.  */
    site = PyImport_ImportModule ("site");
    if (site != NULL)
      {
	/* This macro contains an if; the braces are necessary.  */
	Py_DECREF (site);
      }
  }
  /* END CSL LOCAL */
...
static PyMethodDef GdbMethods[] =
{
  ...
  /* Currently CodeSourcery-local.  */
  { "is_big_endian", gdbpy_is_big_endian, METH_VARARGS,
    "Return whether the current target is big endian" },
