...
struct pipe_state
{
  ...
  /* Whether this serial is for reading stderr.  */
  int is_stderr;
};
...
pipe_windows_open (struct serial *scb, const char *name)
{
  ...
  if (! argv[0] || argv[0][0] == '\0')
    {
      fprintf_unfiltered (gdb_stderr, _("missing child command\n"));
      errno = EINVAL;
      return -1;
    }
  ...
  ps_err = make_pipe_state ();
  make_cleanup (cleanup_pipe_state, ps_err);
...

  {
	...
	/* Our caller expects us to return -1, but all they'll do with
	   ...
	   pex_run, so be verbose here.  */
	if (err)
	  fprintf_unfiltered (gdb_stderr,
			      _("error starting child process '%s': %s: %s\n"),
			      name, err_msg, safe_strerror (err));
	else
	  fprintf_unfiltered (gdb_stderr,
			      _("error starting child process '%s': %s\n"),
			      name, err_msg);
  ...
  if (! pex_stderr)
    goto fail;
  scb->error_scb = serial_for_ops ("pipe's stderr", scb->ops);
  scb->error_scb->fd = fileno (pex_stderr);
  ps_err->output = pex_stderr;
  ps_err->is_stderr = 1;

  scb->state = (void *) ps;

  scb->error_scb->state = (void *) ps_err;
...
pipe_windows_close (struct serial *scb)
{
  ...
  if (ps->is_stderr)
    {
      /* Everything else will be cleared when closing 
	 parent serial.  */
      fclose (ps->output);
    }
  else
    {
      if (scb->error_scb)
	{
	  /* If we call free_pipe_state here, it's possible
	     that the run program is blocked writing something
	     to stderr, which we're not going to read,
	     resulting in deadlock.  Close input and drain
	     output and error before going on.  */
	  while (1)
	    {
	      int ch = serial_readchar (scb, 1);
	      
	      if (_eof (scb->fd) && _eof (scb->error_scb->fd))
		break;
