...
/* For use during unwinding.  Call to record the key location
   for this frame's return address, so that we can compare it
   with other frames to detect an unsaved return address.  */
void frame_set_unwound_pc_location (struct frame_info *this_frame,
...
extern struct frame_info *create_new_current_frame (CORE_ADDR base,
						    CORE_ADDR pc);
...
/* Return 1 if THIS_FRAME saved register REGNUM.  */

extern int frame_saved_register_p (struct frame_info *this_frame, int regnum);
