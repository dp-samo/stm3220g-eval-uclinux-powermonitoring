...
struct frame_info
{
  ...
  /* A location which contributed to the unwound value.  This location
     may not contain the final value, if the target had to adjust it,
     but it should correspond to a unique saved return address; this
     is used to catch frames which do not save the PC.  The default is
     the location information associated with gdbarch_pc_regnum.  */
  struct
  {
    int p;
    enum lval_type lval;
...
/* Create an arbitrary (i.e. address specified by user) or innermost frame
   and make it the current one.  Always returns a non-NULL value.  */

struct frame_info *
create_new_current_frame (CORE_ADDR addr, CORE_ADDR pc)
{
  struct frame_info *fi;

  fi = create_new_frame (addr, pc);
  reinit_frame_cache ();
  current_frame = fi;
...
void
frame_set_unwound_pc_location (struct frame_info *this_frame,
			       int optimized, enum lval_type lval,
			       CORE_ADDR addr, int realnum)
{
  if (!optimized && lval == lval_register)
    frame_register_unwind_location (this_frame->next, realnum,
				    &optimized, &lval, &addr, &realnum);

  this_frame->prev_pc_location.p = 1;
  this_frame->prev_pc_location.optimized = optimized;
  this_frame->prev_pc_location.lval = lval;
  this_frame->prev_pc_location.addr = addr;
  this_frame->prev_pc_location.realnum = realnum;
}

/* Return 1 if THIS_FRAME saved register REGNUM.  If the frame saved the
   register, it will be saved in a different location than it was in the
   next inner frame.  If unsure, return 1; this function is used to
   check for unsaved registers.  */

int
frame_saved_register_p (struct frame_info *this_frame, int regnum)
{
  int optimized, realnum, nrealnum;
  enum lval_type lval, nlval;
  CORE_ADDR addr, naddr;

  frame_register_unwind_location (this_frame, regnum,
				  &optimized, &lval, &addr, &realnum);

  /* Do not use get_next_frame because the sentinel frame is fine
     here.  The current frame saved the register if it is not still in
     its original register.  */
  frame_register_unwind_location (this_frame->next, regnum,
  ...
  if (lval == lval_register)
    return realnum != nrealnum;

  /* We can not be sure, so conservatively assume it is saved.  */
  return 1;
}
...
get_prev_frame_1 (struct frame_info *this_frame)
{
  ...
  if (this_frame->level > 0
      ...
      && frame_unwind_pc (this_frame) == frame_unwind_pc (this_frame->next))
    {
      if (! gdbarch_frame_saved_return_address_p (gdbarch, this_frame))
	{
	  if (frame_debug)
	    {
	      fprintf_unfiltered (gdb_stdlog, "-> ");
...
inside_main_func (struct frame_info *this_frame)
{
  ...
  /* Also check the full symbol table, for the function with the name
     "main" in the user program.  The linker may have changed the
     names of the main function.  For instance, this happens with
     RealView's $Sub$$ and $Super$$ symbols, which are similar to GNU
     ld's --wrap option.  The DejaGNU testsuite wrapper wraps
     main.  */
  symbol = lookup_global_symbol_from_objfile (symfile_objfile, main_name (),
					      VAR_DOMAIN);
  if (symbol == NULL || SYMBOL_CLASS (symbol) != LOC_BLOCK)
    return 0;
  maddr = BLOCK_START (SYMBOL_BLOCK_VALUE (symbol));
  maddr = gdbarch_convert_from_func_ptr_addr (get_frame_arch (this_frame),
					      maddr, &current_target);
  if (maddr == get_frame_func (this_frame))
    return 1;
...
find_frame_sal (struct frame_info *frame, struct symtab_and_line *sal)
{
  ...
  if (frame_inlined_callees (frame) > 0)
    {
      if (inline_call_site (frame, sal))
	return;

      /* If we don't know where the call site is, do not pretend to.
	 This is jarring, but we can't do much better.  Fall back to
