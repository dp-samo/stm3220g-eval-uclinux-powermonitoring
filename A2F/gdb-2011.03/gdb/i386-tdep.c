...
i386_return_value_type (struct gdbarch *gdbarch, struct type *func_type,
			...
			gdb_byte *readbuf, const gdb_byte *writebuf)
{
  ...
  if (code == TYPE_CODE_STRUCT && TYPE_NFIELDS (type) == 1)
    {
      type = check_typedef (TYPE_FIELD_TYPE (type, 0));
      return i386_return_value_type (gdbarch, func_type, type, regcache,
				     readbuf, writebuf);
    }
...
i386_return_value (struct gdbarch *gdbarch, struct type *func_type,
		   ...
		   gdb_byte *readbuf, const gdb_byte *writebuf)
{
  return i386_return_value_type (gdbarch, func_type, value_type (function),
...
/* This is marked no inline to prevent a false warning from the 4.3
   build tools.  They incorrectly claim a possible type-based punning
   aliasing problem.  However, that looks to be a compiler bug that is
   very sensitive to the environment in which the build compiler is
   invoked, including it going away when using preprocessed source or
   the debugger.  */
