...
static enum return_value_convention
m68k_svr4_return_value_type (struct gdbarch *gdbarch, struct type *func_type,
  ...
  if (code == TYPE_CODE_STRUCT && TYPE_NFIELDS (type) == 1)
    {
      ...
      return m68k_svr4_return_value_type (gdbarch, func_type, type, regcache,
					  readbuf, writebuf);
    }
...
m68k_svr4_return_value (struct gdbarch *gdbarch, struct type *func_type,
			...
			gdb_byte *readbuf, const gdb_byte *writebuf)
{
  return m68k_svr4_return_value_type (gdbarch, func_type,
				      value_type (function), regcache,
				      readbuf, writebuf);
}

