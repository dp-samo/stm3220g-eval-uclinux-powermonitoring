...
typedef unsigned32	netbsd_uid_t;
typedef unsigned32	netbsd_gid_t;
typedef unsigned32	netbsd_dev_t;
typedef unsigned64	netbsd_ino_t;

struct netbsd_stat {
  netbsd_dev_t nbsd_st_dev;         /* device */
  unsigned32 nbsd_st_mode;        /* protection */
  netbsd_ino_t nbsd_st_ino;         /* inode */
  unsigned32 nbsd_st_nlink;       /* number of hard links */
  netbsd_uid_t nbsd_st_uid;         /* user ID of owner */
  netbsd_gid_t nbsd_st_gid;         /* group ID of owner */
  netbsd_dev_t nbsd_st_rdev;        /* device type */

  signed32 nbsd_st_atime;        /* time of last access */
  signed32 nbsd_st_atimensec;
  signed32 nbsd_st_mtime;        /* time of last modification */
  signed32 nbsd_st_mtimensec;
  signed32 nbsd_st_ctime;        /* time of last change */
  signed32 nbsd_st_ctimensec;

  unsigned64 nbsd_st_size;        /* total size, in bytes */
  signed64 nbsd_st_blocks;       /* number of blocks allocated */
  unsigned32 nbsd_st_blksize;      /* blocksize for filesystem I/O */
  unsigned32 nbsd_st_flags;       /* user-defined flags */
  unsigned32 nbsd_st_gen;         /* filesystem generation number */
  unsigned32 nbsd_st_spare[2];
};
...
write_stat(unsigned_word addr,
	   ...
	   unsigned_word cia)
{
  struct netbsd_stat target;

  H2T(buf.st_dev);
  target.nbsd_st_dev = buf.st_dev;
  H2T(buf.st_ino);
  target.nbsd_st_ino = buf.st_ino;
  H2T(buf.st_mode);
  target.nbsd_st_mode = buf.st_mode;
  H2T(buf.st_nlink);
  target.nbsd_st_nlink = buf.st_nlink;
  H2T(buf.st_uid);
  target.nbsd_st_uid = buf.st_uid;
  H2T(buf.st_gid);
  target.nbsd_st_gid = buf.st_gid;
  H2T(buf.st_size);
  target.nbsd_st_size = buf.st_size;

#ifdef AC_STRUCT_ST_RDEV
  H2T(buf.st_rdev);
  target.nbsd_st_rdev = buf.st_rdev;
#else
  target.nbsd_st_rdev = 0;
#endif

  H2T(buf.st_atime);
  target.nbsd_st_atime = buf.st_atime;
  H2T(buf.st_mtime);
  target.nbsd_st_mtime = buf.st_mtime;
  H2T(buf.st_ctime);
  target.nbsd_st_ctime = buf.st_ctime;

#ifdef AC_STRUCT_ST_BLKSIZE
  H2T(buf.st_blksize);
  target.nbsd_st_blksize = buf.st_blksize;
#else
  target.nbsd_st_blksize = 0;
#endif
#ifdef AC_STRUCT_ST_BLOCKS
  H2T(buf.st_blocks);
  target.nbsd_st_blocks = buf.st_blocks;
#else
  target.nbsd_st_blocks = 0;
#endif
#if WITH_NetBSD_HOST
  H2T(buf.st_flags);
  target.nbsd_st_flags = buf.st_flags;
  H2T(buf.st_gen);
  target.nbsd_st_gen = buf.st_gen;
#endif
...
/* Copied from <sys/ttycom.h>.  */

#define NETBSD_TIOCGETA _IOR('t', 19, struct netbsd_termios) /* get termios struct */

/* Copied from <sys/termios.h>.  */

typedef unsigned32 netbsd_tcflag_t;
typedef unsigned char netbsd_cc_t;
/* This definition is wonky; the speed_t typedef in termios.h is
   'unsigned int', but the struct termios usage is 'int'; we go with
   what's actually used in the structure, because that's all we care
   about.  */
typedef signed32 netbsd_speed_t;
...
#if !WITH_NetBSD_HOST
  /* We use TIOCGETA for our isatty semihosting support.  All we really
     care about is whether isatty succeeds, so we don't bother to set
     errno.  */
  if (request == NETBSD_TIOCGETA)
    cpu_registers(processor)->gpr[arg0] = isatty(d) ? 0 : -1;
  else
...
static emul_syscall_descriptor netbsd_descriptors[] = {
  ...
  /* 218 */ { 0, "lkmnosys" },
  /* 219 */ { 0, "lkmnosys" },
  /* 220 */ { 0, "compat_14_semctl" },
  /* 221 */ { 0, "semget" },
  /* 222 */ { 0, "semop" },
  /* 223 */ { 0, "semconfig" },
  /* 224 */ { 0, "compat_14_msgctl" },
  /* 225 */ { 0, "msgget" },
  /* 226 */ { 0, "msgsnd" },
  /* 227 */ { 0, "msgrcv" },
  /* 228 */ { 0, "shmat" },
  /* 229 */ { 0, "compat_14_shmctl" },
  /* 230 */ { 0, "shmdt" },
  /* 231 */ { 0, "shmget" },
  /* 232 */ { 0, "clock_gettime" },
  /* 233 */ { 0, "clock_settime" },
  ...
  /* 289 */ { 0, "preadv" },
  /* 290 */ { 0, "pwritev" },
  /* 291 */ { 0, "compat_16___sigaction14" },
  /* 292 */ { 0, "__sigpending14" },
  /* 293 */ { 0, "__sigprocmask14" },
  /* 294 */ { 0, "__sigsuspend14" },
  /* 295 */ { 0, "compat_16___sigreturn14" },
  /* 296 */ { 0, "__getcwd" },
  /* 297 */ { 0, "fchroot" },
  /* 298 */ { 0, "compat_30_fhopen" },
  /* 299 */ { 0, "compat_30_fhstat" },
  /* 300 */ { 0, "compat_20_fhstatfs" },
  /* 301 */ { 0, "____semctl13" },
  /* 302 */ { 0, "__msgctl13" },
  /* 303 */ { 0, "__shmctl13" },
  /* 304 */ { 0, "lchflags" },
  ...
  /* Syscalls 318-329 reserved for implementing Solaris-compatible LWP calls.  */
  { 0, }, /* 318 */
  { 0, }, /* 319 */
  { 0, }, /* 320 */
  { 0, }, /* 321 */
  ...
  /* 334 */ { 0, "sa_yield" },
  /* 335 */ { 0, "sa_preempt" },
  { 0, }, /* 336 is obsolete sys_sa_unblockyield */
  /* Sycalls 337-339 reserved for implementing scheduler activations syscalls.  */
  { 0, }, /* 337 */
  { 0, }, /* 338 */
  { 0, }, /* 339 */
  ...
  /* 344 */ { 0, "kqueue" },
  /* 345 */ { 0, "kevent" },
  /* 346 */ { 0, "sched_setparam"},
  /* 347 */ { 0, "sched_getparam"},
  /* 348 */ { 0, "sched_setscheduler"},
  /* 349 */ { 0, "sched_getscheduler"},
  /* 350 */ { 0, "sched_yield"},
  /* 351 */ { 0, "sched_get_priority_max"},
  /* 352 */ { 0, "sched_get_priority_min"},
  /* 353 */ { 0, "sched_rr_get_interval"},
  /* 354 */ { 0, "fsync_range" },
  /* 355 */ { 0, "uuidgen" },
  /* 356 */ { 0, "getvfsstat" },
  /* 357 */ { 0, "statvfs1" },
  /* 358 */ { 0, "fstatvfs1" },
  /* 359 */ { 0, "compat_30_fhstatvfs1" },
  /* 360 */ { 0, "extattrctl" },
  ...
  /* 385 */ { 0, "lremovexattr" },
  /* 386 */ { 0, "fremovexattr" },
  /* 387 */ { do_stat, "__stat30" },
  /* 388 */ { do_fstat, "__fstat30" },
  /* 389 */ { do_lstat, "__lstat30" },
  /* 390 */ { 0, "__getdents30" },
  /* 391 */ { 0, "posix_fadvise" },
  /* 392 */ { 0, "compat_30___fhstat30" },
  /* 393 */ { 0, "__ntp_gettime30" },
  /* 394 */ { 0, "__socket30" },
  /* 395 */ { 0, "__getfh30" },
  /* 396 */ { 0, "__fhopen40" },
