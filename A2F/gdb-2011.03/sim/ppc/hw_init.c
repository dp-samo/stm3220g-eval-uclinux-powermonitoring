...
static void
update_for_binary_phdr(bfd *abfd,
		       ...
		       PTR obj)
{
  ...
  DTRACE(binary,
	 ("LOAD, lma=0x%.8lx, vma=0x%.8lx, filesize=%6ld, memsize=%6ld\n",
	  (long)phdr->p_paddr,
	  (long)phdr->p_vaddr,
	  (long)phdr->p_filesz,
	  (long)phdr->p_memsz));
  ...
  if (device_find_property(me, "claim") != NULL) {
    ...
    unsigned_cell mem_out[1];
    mem_in[0] = 0; /*alignment - top-of-stack*/
    mem_in[1] = phdr->p_memsz;
    mem_in[2] = phdr->p_paddr;
    if (device_instance_call_method(memory, "claim", 3, mem_in, 1, mem_out) < 0)
      device_error(me, "failed to claim memory for segment at 0x%lx (0x%lx)",
		   (long) phdr->p_paddr, (long) phdr->p_memsz);
    if (mem_out[0] != phdr->p_paddr)
      device_error(me, "section address not as requested");
  }
  ...
  /* if a load dma in the required data */
  if (phdr->p_filesz > 0) {
    void *section_init = zalloc(phdr->p_filesz);
    if (bfd_seek(abfd, phdr->p_offset, SEEK_SET) != 0
	|| bfd_bread(section_init, phdr->p_filesz, abfd) != phdr->p_filesz) {
      bfd_perror("binary");
      device_error(me, "load of data failed");
      return;
    }
...
hw_binary_init_data_callback(device *me)
{
  ...
  /* If there are segments, use them.  */
  phdr_bytes = bfd_get_elf_phdr_upper_bound (image);
  if (phdr_bytes > 0)
    {
      Elf_Internal_Phdr *phdrs = zalloc (phdr_bytes);
      int i, num_phdrs;
      num_phdrs = bfd_get_elf_phdrs (image, phdrs);
      for (i = 0; i < num_phdrs; i++)
	update_for_binary_phdr (image, &phdrs[i], (PTR) me);
      zfree (phdrs);
    }
  else
    /* Otherwise map each section.  */
