...
/* Address for mdi semihosting breakpoint, if non-zero */
static SIM_ADDR mdi_semihost_bkpt = 0;
     ...
     char **env;
{
  ...
  if (abfd != NULL)
    {
      /* Look for .sdeosabi section, which contains a pointer to the
         _mdi_syscall function.  Put a breakpoint there.  */
      asection *sec = bfd_get_section_by_name (abfd, ".sdeosabi");

      if (sec && bfd_get_section_size (sec) == 8)
	{
	  sim_cpu *cpu = STATE_CPU (sd, 0);
	  SIM_ADDR addr = bfd_section_lma (abfd, sec);
	  
	  /* The first word is a hard coded '2'.  I'm guessing it is a
	     version number.  The second word is the address of the
	     syscall.  */
	  unsigned32 bkpt_info[2];

	  if (sim_read (sd, addr, (char *)&bkpt_info, 8) == 8)
	    {
	      T2H (bkpt_info[0]);
	      T2H (bkpt_info[1]);
		...
		{
		  unsigned32 bkpt_insn = BREAKPOINT_INSTRUCTION;

		  H2T (bkpt_insn);
		  mdi_semihost_bkpt = bkpt_info[1];
		  sim_write (sd, mdi_semihost_bkpt, (char *)&bkpt_insn, 4);
#ifdef DEBUG
		  printf ("DBG: found mdi semihosting at 0x%s\n",
			  pr_addr (mdi_semihost_bkpt));
#endif
		}
	    }
	}
		 ...
		 int *exit_code)
{
  ...
  switch (V0)
    ...
    case 0: /* OPEN */
      /* A0 points to a NUL terminated string,
	 A1 is flags
	 A2 is mode
      */
      {
	char *path = fetch_str (sd, A0);
	/* No way to pass mode. */
	result = sim_io_open (sd, path, (int)A1);
	zfree (path);
      }
      break;
    ...
    case 1: /* CLOSE */
      /* A0 is handle */
      result = sim_io_close (sd, (int)A0);
      break;
      
    case 2: /* READ */
      /* A0 is handle,
	 A1 is pointer to buffer,
	 A2 is number of bytes
      */
      {
	char *buf = zalloc (A2);
	result = sim_io_read (sd, A0, buf, A2);
	if (result > 0)
	  sim_write (sd, A1, buf, result);
	zfree (buf);
      }
      break;
    ...
    case 3: /* WRITE */
      ...
      */
      {
	char *buf = zalloc (A2);
	sim_read (sd, A1, buf, A2);
	result = sim_io_write (sd, A0, buf, A2);
	if (A0 == 1)
	  sim_io_flush_stdout (sd);
	else if (A0 == 2)
	  sim_io_flush_stderr (sd);
    ...
    case 6: /* LSEEK32 */
      ...
      */
      {
	result = sim_io_lseek (sd, A0, A1, A2);
      }
      break;
      
    case 7: /* GETTIME of day */
      /* Result goes in V0 (sec) & V1 (usec) */
      {
	struct timeval tv;
		  ...
		  int exception,...)
{
  ...
  switch (exception) {
	...
	{
	  /* check CIA maps to the physical address of the
	     breakpoint.  */
	  address_word paddr;
	  int uncached;

	  AddressTranslation (cia, isINSTRUCTION, isLOAD,
			      &paddr, &uncached, isTARGET, isREAL);
	  if (paddr == mdi_semihost_bkpt)
	    {
	      int exit_code = 0;
#ifdef DEBUG
	      printf ("DBG: semihosting SYSCALL %lu\n", (unsigned long)V0);
#endif
	      /* At this point $v0 has the syscall number, $a0..a1 have
	         argument values.  Put the return value in $v0 and errno in
		 $v1, and then jump to the $ra value.  */
	      if (mdi_semihosting (SD, CPU, &exit_code))
		/* Halt the simulator.  */
		sim_engine_halt (SD, CPU, NULL, NULL_CIA, sim_exited,
				 exit_code);
	      else
		{
		  /* Continue from the return address.  We have to
		     restart, otherwise we'll unconditionally execute
