...
( echo "#include <asm/unistd.h>";
  ...
  sed -ne 's/^[ ]*#define[ ]*\(__ARM_NR_\|__NR_\|SYS_ARM\)\([A-Za-z0-9_]*\).*/UCLIBC\1\2 \1\2/gp' \
      -e 's/^[ ]*#undef[ ]*\(__ARM_NR_\|__NR_\|SYS_ARM\)\([A-Za-z0-9_]*\).*/UNDEFUCLIBC\1\2 \1\2/gp' # needed to strip out any kernel-internal defines
) |
$CC -E $INCLUDE_OPTS - |
( echo "/* WARNING!!! AUTO-GENERATED FILE!!! DO NOT EDIT!!! */" ;
  ...
  echo "#endif" ; echo ;
  sed -ne 's/^UCLIBC\(__ARM_NR_\|__NR_\|SYS_ARM\)\([A-Za-z0-9_]*\) *\(.*\)/#undef \1\2\
#define \1\2 \3\
#define SYS_\2 \1\2/gp' \
     -e 's/^UNDEFUCLIBC\(__ARM_NR_\|__NR_\|SYS_ARM\)\([A-Za-z0-9_]*\).*/#undef \1\2/gp'
  echo ;
  echo "#endif" ;
)
